        ...
        <Card padded={false}>
          ...
          {!loading && items.length > 0 && (
            <div className="divide-y divide-surface-border">
              {items.map((item, idx) => (
                <div key={`${item.lead.recordId}-${item.issueType}-${idx}`} className="flex items-center justify-between px-5 py-3.5 hover:bg-surface-hover transition-colors">
                  <div className="flex items-center gap-3 min-w-0 cursor-pointer" onClick={() => navigate(`/leads/${item.lead.recordId}`)}>
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-ink-faint">{item.lead.contactDetails || 'No contact'} · {item.lead.leadStage || 'New'} · {item.lead.leadManagedBy || 'Unassigned'}</p>
                    </div>
                  </div>
